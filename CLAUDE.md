...
A light/dark toggle sits in the top-right of the nav (`.theme-toggle` button, with inline sun/moon SVGs swapped by CSS). Theming is driven by a `data-theme` attribute on `<html>`: the dark palette is a single `html[data-theme="dark"]{...}` block that overrides the `:root` color variables, plus a handful of explicit overrides for hardcoded colors that don't read from variables (nav background, the body grays, `.chip-tag`/`.btn-ghost`, `.dot`, the socials tooltip whose background is `var(--ink)` and would otherwise invert, and `.nav-btn` white backgrounds). That dark block is part of the duplicated inline `<style>` and follows the same "change one, change all" rule as the rest of the chrome. The lone exception mirrors the existing `#projects` convention: all `#projects`-scoped overrides live only on `projects.html` (the light `#projects .logo-chip`/`#projects .loc` rules and the dark `html[data-theme="dark"] #projects .loc` override), so `index.html` and `education.html` keep byte-identical style blocks.
