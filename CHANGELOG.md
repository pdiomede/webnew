...
## [1.2.0] - 2026-08-04
- **Changed:** current role updated to Customer Success Manager @ Hypernative (August 2026 – Present) across the splash, hero meta/`<title>`, Open Graph / Twitter tags, and the `Person` JSON-LD; the previous Technical Account Manager @ Certora role (January 2026 – July 2026) moved into the Journey carousel as its newest entry. The Current Role card carries the new role's description and focus pills (Customer Success, Onboarding & Adoption, Retention & Renewals, Stakeholder Management, Security Strategy).
...
- **Added:** avatar pointer-follow on all three pages - the hero profile badge tilts in 3D and shifts toward the mouse pointer, casting a trailing drop shadow that drifts opposite the pull and deepens as the badge lifts (new shared `js/avatar.js`: rAF-throttled document `mousemove`, up to 10px pull, 1.04 scale, and 15° tilt eased within 220px of the face, reset when the pointer leaves the page). Skipped on touch / coarse-pointer devices and under `prefers-reduced-motion` (gated in JS), and idle while the landing splash is open; one shared CSS line transitions the transform and shadow.
- **Added:** breaching dolphins in the hero banner on all three pages - two flat navy silhouettes (pure CSS/SVG, no JS) leap out of the water between the mid and front wave layers, arc nose-up to nose-down, and dive back behind the front wave while the banner is hovered. They alternate on offset 2s cycles, so a new breach starts every second (the first about 1s into hover) with exactly one dolphin airborne at a time. The hover play-state rule and its reduced-motion re-pause now list seven selectors.
...
- **Fixed:** the splash previous-roles line stays on one line - fluid font size (14px on wide screens, shrinking to 11px near 900px viewports, wrapping normally below that); the splash content column widened from 680px to 900px to give it room.
...
- **Fixed:** removed the dead Clearbit hop from the logo fallback chain - `logo.clearbit.com` was discontinued and every `domain:` logo request failed before falling back; Google favicons are now fetched directly.
- **Fixed:** refreshed the stale `sitemap.xml` lastmod dates (2026-06-13 → 2026-08-04).
- **Docs:** aligned the changed `js/*.js` cache-busters to `1.2.0`; updated README, CLAUDE.md, and COLOR_PALETTE.md.
